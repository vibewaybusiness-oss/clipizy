...
class TrapNationParticles:
    ...
    - Particles disappear outside frame and respawn from center
    ...
    def _update_particles(self, t, frame_idx):
        ...
        for particle in self.particles:
            ...
            # Check if particle is outside frame and respawn from center
            if (particle['x'] < 0 or particle['x'] >= self.W or 
                particle['y'] < 0 or particle['y'] >= self.H):
                # Respawn from center with outward movement
                particle['x'] = self.cx
                particle['y'] = self.cy
                # Set outward velocity from center
                angle = np.random.uniform(0, 2 * np.pi)
...
def test_trap_nation_particles():
    ...
    print(f"   - Particles respawn from center when leaving frame")

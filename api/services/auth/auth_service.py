...
class AuthService:
    ...
    def create_user(self, db: Session, user: UserCreate) -> User:
        """Create a new user with complete setup using unified onboarding"""
        logger.info(f"Creating new user: {user.email}")
        try:
            from api.services.auth.unified_onboarding_service import unified_onboarding_service
            
            # Use unified onboarding service for email users
            db_user = unified_onboarding_service.onboard_user(
                db=db,
                email=user.email,
                name=user.name,
                password=user.password,
                oauth_provider=None  # Email user
            )
            logger.info(f"User created successfully with unified onboarding: {user.email} (ID: {db_user.id})")
            return db_user

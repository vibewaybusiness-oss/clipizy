...
import type { MusicTrack, TrackDescriptions, TrackGenres } from '@/types/domains';

export function useMusicTracks(projectId?: string | null, markAsChanged?: () => void) {
  ...
  // PERSISTENCE: Save state changes to localStorage and schedule backend sync with debouncing
  const saveTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  
  useEffect(() => {
    if (typeof window !== 'undefined' && projectId && isInitialized) {
      // Clear existing timeout
      if (saveTimeoutRef.current) {
        clearTimeout(saveTimeoutRef.current);
      }
      
      // Debounce localStorage saves to reduce I/O operations
      saveTimeoutRef.current = setTimeout(() => {
        // Create a serializable version of musicTracks for localStorage
        ...
        // Mark as changed when tracks data changes
        if (markAsChanged) {
          markAsChanged();
        }
  ...
  }, [musicTracks, selectedTrackId, selectedTrackIds, trackDescriptions, trackGenres, projectId, isInitialized]);


  const addTracks = useCallback((newTracks: MusicTrack[]) => {
    console.log('🎵 addTracks called with:', newTracks);
    console.log('🎵 Current musicTracks before adding:', musicTracks);
    
    setMusicTracks(prev => {
      const updated = [...prev, ...newTracks];
      console.log('🎵 Updated musicTracks after adding:', updated);
      return updated;
    });

    // Set the first track as selected if none is selected
    if (!selectedTrackId && newTracks.length > 0) {
      console.log('🎵 Setting first track as selected:', newTracks[0].id);
    ...
    console.log('🎵 addTracks completed');
  }, [selectedTrackId, toast, musicTracks]);
  ...
  // Check if a track needs to be uploaded (has File object and not uploaded)
  const needsUpload = useCallback((track: MusicTrack): boolean => {
    return !!(track.file && track.file instanceof File && !track.uploaded);
  }, []);

  // Get tracks that need upload
  const getTracksNeedingUpload = useCallback((): MusicTrack[] => {
    return musicTracks.filter(needsUpload);
  }, [musicTracks, needsUpload]);

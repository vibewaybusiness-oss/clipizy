...
const trackBlobUrl = (url: string, source: string = 'unknown') => {
  if (process.env.NODE_ENV === 'development' && url.startsWith('blob:')) {
...
export function useAudioPlayback() {
  ...
  const isValidAudioUrl = (url: string): boolean => {
    ...
    try {
      ...
      // If it's a relative path, it might be valid (will be converted to full URL)
  ...
  const playTrack = useCallback(async (track: { id: string; file: File; url: string }) => {
    ...
    if (!track.url || track.url === '') {
      ...
      if (track.file && track.file.size > 0) {
        ...
        if (!track.file || (!(track.file as any instanceof File) && !(track.file as any instanceof Blob))) {
      ...
      if (audioUrl && !audioUrl.startsWith('http') && !audioUrl.startsWith('blob:')) {
        ...
        console.log(`Converted relative URL to full URL: ${track.url} -> ${audioUrl}`);
      }

      // Validate the URL format
      if (!isValidAudioUrl(audioUrl)) {
        console.error('Invalid audio URL format for track:', track.id, 'URL:', audioUrl);
        toast({
          variant: "destructive",
          title: "Playback Error",
      ...
      if (audioUrl.startsWith('blob:')) {
        console.log('Testing blob URL accessibility for track:', track.id, 'URL:', audioUrl);
        ...
        console.log('Using non-blob URL for track:', track.id, 'URL:', audioUrl);
      }

      // Check for common CORS or accessibility issues
      if (audioUrl.startsWith('http') && !audioUrl.includes(window.location.origin)) {
        console.warn('Cross-origin audio URL detected - may have CORS issues:', audioUrl);
      }
    }

...
export async function generateImage(
  ...
  try {
    ...
    const response = await fetch(`${getBackendUrl()}/comfyui/generate`, {
      method: 'POST',
      headers: {
...
export async function downloadImage(
  ...
  try {
    ...
    const response = await fetch(
      `${getBackendUrl()}/comfyui/download?filename=${encodeURIComponent(filename)}&subfolder=${encodeURIComponent(subfolder)}&type=${encodeURIComponent(type)}`,
      {
        method: 'GET',
      }
    );
...
export async function getWorkflowStatus(promptId: string): Promise<NextResponse> {
  try {
    const response = await fetch(
      `${getBackendUrl()}/comfyui/status/${promptId}`,
      {
        method: 'GET',
      }
    );
...
export async function getAvailableWorkflows(): Promise<NextResponse> {
  try {
    const response = await fetch(
      `${getBackendUrl()}/comfyui/workflows`,
      {
        method: 'GET',
      }
    );

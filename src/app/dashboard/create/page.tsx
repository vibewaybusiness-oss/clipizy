...
import { ProjectSelectionPopup } from "@/components/forms/music-clip/ProjectSelectionPopup";
...
export default function CreatePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center p-4 sm:p-8">
      <div className="space-y-6 sm:space-y-8">
        ...
        {/* CREATION OPTIONS */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-6xl mx-auto justify-items-center">
        {creationOptions.map((option) => {
          const Icon = option.icon;
          return (
            <Card
              key={option.id}
              className={`bg-card clickable-card group relative flex flex-col w-full max-w-sm ${
                option.popular ? 'ring-2 ring-primary/20' : ''
              } ${option.id === "script-video" ? 'relative' : ''}`}
            >
              {option.popular && (
                <div className="absolute -top-2 sm:-top-3 left-1/2 transform -translate-x-1/2">
                  <Badge className="bg-primary text-white px-2 sm:px-3 py-1 text-xs font-medium">
                    Most Popular
                  </Badge>
                </div>
              )}
              ...
              {option.id === "script-video" && (
                <div className="absolute inset-0 bg-black/60 z-10 flex items-center justify-center rounded-lg">
                  <div className="text-center p-4">
                    <h3 className="text-lg sm:text-2xl font-bold text-white mb-2">Available Soon</h3>
                    <p className="text-white/80 text-xs sm:text-sm">We're working on this feature</p>
                  </div>
                </div>
              )}
              ...
              {option.id === "script-video" ? (
                <div className="flex flex-col h-full">
                  <CardHeader className="text-center pb-3 sm:pb-4 flex-shrink-0">
                    <div className={`w-16 h-16 sm:w-20 sm:h-20 ${option.color} rounded-2xl flex items-center justify-center mx-auto mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-200 shadow-lg`}>
                      <Icon className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
                    </div>
                    <CardTitle className="text-xl sm:text-2xl font-bold text-foreground group-hover:text-primary transition-colors mb-2 sm:mb-3">
                      {option.title}
                    </CardTitle>
                  ...
                  <CardContent className="pt-0 flex flex-col flex-grow">
                    <div className="space-y-4 sm:space-y-6 flex-grow">
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        {option.features.map((feature, index) => (
                          <div key={index} className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm text-muted-foreground">
                      ...
                      {/* METADATA */}
                      <div className="flex items-center justify-center text-xs sm:text-sm text-muted-foreground pt-2 border-t border-border">
                    ...
                    <Button
                      className="w-full mt-4 sm:mt-6 h-10 sm:h-12 text-sm sm:text-base font-semibold bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white transition-all duration-200 relative z-20"
                    >
                      Notify Me
                      <Mail className="w-4 h-4 sm:w-5 sm:h-5 ml-2 group-hover:scale-110 transition-transform" />
                    </Button>
                  </CardContent>
                </div>
                ...
                <div className="flex flex-col h-full cursor-pointer" onClick={handleMusicClipClick}>
                  ...
                  <CardContent className="pt-0 flex flex-col flex-grow">
                    ...
                    {/* CTA BUTTON - Always at bottom */}
                    <Button
                      className={`w-full mt-4 sm:mt-6 h-10 sm:h-12 text-sm sm:text-base font-semibold ${option.popular ? 'bg-primary hover:bg-primary/90 text-white' : 'bg-muted hover:bg-muted/80 text-foreground'} transition-all duration-200`}
                    >
                      Create {option.title}

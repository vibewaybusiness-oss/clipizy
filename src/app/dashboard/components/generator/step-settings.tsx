...
import { RadioGroup, RadioGroupItem } from "@/app/dashboard/components/ui/radio-group";
...
import { BudgetSlider } from "@/app/dashboard/components/budget-slider";
import { SettingsSchema, calculateLoopedBudget, calculateScenesBudget } from "@/app/dashboard/components/vibewave-generator";
import { usePricing } from "@/hooks/use-pricing";
import { CONFIG } from "@/lib/config";
import { Label } from "@/app/dashboard/components/ui/label";
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle, SheetTrigger } from "@/app/dashboard/components/ui/sheet";
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from "@/app/dashboard/components/ui/dialog";
...
import { SelectionModal } from "@/app/dashboard/components/ui/selection-modal";
...
export function StepSettings({ form, audioDuration, totalDuration, trackCount, trackDurations, onSubmit, onBack, hideNavigation = false }: StepSettingsProps) {
  ...
  // Note: Removed automatic validation trigger to prevent showing errors from the start
  
  const loopedVideoCost = useMemo(() => {
    if (!pricing) return 0; // Don't calculate if pricing not loaded
    return calculateLoopedBudget(totalDuration, trackCount, pricing, reuseVideo, videoType as 'looped-static' | 'looped-animated');
  }, [totalDuration, trackCount, pricing, reuseVideo, videoType]);

  const scenesVideoCost = useMemo(() => {
    if (!pricing) return 0; // Don't calculate if pricing not loaded
    return calculateScenesBudget(totalDuration, trackDurations, pricing, reuseVideo);
  }, [totalDuration, trackDurations, pricing, reuseVideo]);

  // Set the budget to the appropriate value by default when the component loads or type changes.
  useEffect(() => {
    if (!pricing) return; // Don't set budget if pricing not loaded
    
    const currentBudget = form.getValues('budget')?.[0] || 0;
    
    let initialBudget;
    if (videoType === 'looped-static' || videoType === 'looped-animated') {
        // For static/animated, use fixed cost
        initialBudget = loopedVideoCost;
    } else { // scenes
        // For scenes, use the calculated cost as the initial value (slider will handle max)
        initialBudget = scenesVideoCost;
    }
    
    // Only update if the budget is significantly different to prevent infinite loops
    // Also check if we're not in the middle of a form update to prevent loops
    if (initialBudget !== undefined && 
        initialBudget > 0 && 
        Math.abs(initialBudget - currentBudget) > 1 &&
        !form.formState.isSubmitting) {
      form.setValue('budget', [initialBudget], { shouldValidate: false, shouldDirty: false });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [videoType, pricing]);

  // Note: Auto-continue logic removed to allow users to see and use the continue button
  ...
  if (pricingError || !pricing) {
    return (
      <div className="w-full animate-fade-in-up">
        <div className="flex items-center justify-center p-8">
          <div className="text-center">
            <p className="text-destructive mb-4">Error loading pricing configuration</p>
              ...
              <SelectionModal
                ...
                options={[
                  ...
                  },
                  {
                    ...
                    cost: pricing ? scenesVideoCost : 0,

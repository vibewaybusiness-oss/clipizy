...
import { Music, ArrowLeft, ChevronLeft, ChevronRight, Upload, Film, Trash2, Loader2 } from "lucide-react";
...
import { SettingsSchema, PromptSchema, OverviewSchema } from "@/components/forms/generators/ClipizyGenerator";
import * as z from "zod";
import WaveformVisualizer, { type WaveformVisualizerRef } from "@/components/features/music-clip/waveform-visualizer";
import { StepUpload } from "@/components/forms/StepMusic";
import { StepSettings } from "@/components/forms/StepVideo";
import { StepPrompt } from "@/components/forms/StepPrompt";
import { StepOverview } from "@/components/forms/StepSettings";
...
const MusicClipPage = React.memo(function MusicClipPage() {
  ...
  }, [musicClipState.state.currentStep, projectId, musicAnalysisData, loadAnalysisData, toast]);
  ...
  useEffect(() => {
    const restoreAudioState = async () => {
      if (musicTracks.selectedTrackId && musicTracks.musicTracks.length > 0) {
        ...
        if (selectedTrack && selectedTrack.file && selectedTrack.file instanceof File && selectedTrack.file.size > 0) {
  ...
  const pushDataToBackend = async (projectId: string, musicClipData: any, tracksData: any) => {
    try {
      ...
      if (musicAnalysis.analysisData) {
        try {
          await projectsAPI.updateProjectAnalysis(projectId, musicAnalysis.analysisData);
  ...
  const handleAudioFileChange = async (files: File[]) => {
    ...
    try {
      ...
      if (audioFiles.length > 1) {
        const batchResult = await projectsAPI.uploadTracksBatch(projectId, audioFiles, {
          ai_generated: false,
          instrumental: false,
        });
        ...
        try {
          const uploadResult = await projectsAPI.uploadTrack(projectId, file, {
            ai_generated: false,
            instrumental: false,
          });
  ...
  if (musicClipState.state.currentStep === 4) {
    return (
      <OverviewLayout
        ...
        isLoading={musicClipState.state.isNavigating}
      />
    );
  }
                  ...
                  <div className="flex items-center justify-between">
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (musicAnalysis.analysisData && projectManagement.state.currentProjectId) {
                                try {
                                  await projectsAPI.updateProjectAnalysis(projectManagement.state.currentProjectId, musicAnalysis.analysisData);
                                  console.log('Analysis data saved to project');
                                } catch (error) {
                                  console.error('Failed to save analysis data:', error);
                                }
                    ...
                    {musicClipState.state.currentStep === 1 && (
                      <Button
                        ...
                        disabled={musicTracks.musicTracks.length === 0 || !musicTracks.selectedTrackId || musicClipState.state.isNavigating}
                      >
                        {musicClipState.state.isNavigating ? (
                          <>
                            <Loader2 className="w-4 h-4 animate-spin" />
                    ...
                    {musicClipState.state.currentStep === 2 && (
                      <Button
                        ...
                        disabled={!musicClipState.forms.settingsForm.formState.isValid || musicClipState.state.isNavigating}
                      >
                        {musicClipState.state.isNavigating ? (
                          <>
                            <Loader2 className="w-4 h-4 animate-spin" />

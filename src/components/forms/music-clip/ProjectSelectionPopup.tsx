        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {authLoading || loading ? (
              ...
              <div className="grid gap-2 max-h-96 overflow-y-auto scrollbar-modern">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            <span className="inline-flex items-center rounded-full border border-transparent bg-secondary text-secondary-foreground text-xs px-1.5 py-0.5 font-semibold">
                              {project.status}
                            </span>
                          </div>
                          ...
                          {/* Track Preview - Only show if there are tracks */}
                          {project.tracks && project.tracks.length > 0 && (
                            <div className="mt-2">
                              <div className="flex items-center space-x-2 text-xs">
                                {project.tracks.slice(0, 2).map((track, index) => (
                                  <div key={track.id} className="flex items-center space-x-1">
                                    <span className="truncate max-w-24">{track.name}</span>
                                    <span className="text-muted-foreground">
                                      {formatDuration(track.duration)}
                                    </span>
                                    {index < Math.min(project.tracks!.length, 2) - 1 && <span className="text-muted-foreground">•</span>}
                                  </div>
                                ))}
                        ...
                        <div className="flex items-center space-x-1 ml-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={(e: React.MouseEvent) => handleDeleteClick(e, project)}

...
import { getScenesInfo } from "@/components/forms/music-clip/ClipiziGenerator";
import { usePricing, usePricingService } from "@/hooks";
...
export function BudgetSlider({
  ...
  const videoDurationFromConfig = (pricing?.video_generator?.['clipizy-model']?.['video-duration'] || 5) / 60; // Get from config and convert to minutes
  ...
  const costPerScene = useMemo(() => {
    if (videoType === 'scenes' && pricing) {
      const modelConfig = pricing.video_generator['clipizy-model'];

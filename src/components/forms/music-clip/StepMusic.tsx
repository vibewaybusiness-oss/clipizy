...
import type { MusicTrack, GenerationMode } from "@/types/domains/music";
...
export function StepUpload({
  ...
  return (
    ...
    <div className="space-y-1">
      <h3 className="text-lg font-semibold">Upload or generate music</h3>
    </div>
          {/* Info Banner */}
          ...
          <div className="space-y-6 flex-1 flex flex-col">
            ...
            >
              <div className="flex flex-col items-center justify-center p-4 rounded-xl cursor-pointer border-2 border-dashed border-border hover:border-muted-foreground/50 transition-all duration-300 bg-muted/30 hover:bg-muted/50 group">
                <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center mb-2 group-hover:scale-110 transition-transform duration-300 mx-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                <textarea
                  ...
                  className="min-h-[80px] resize-none text-base w-full px-3 py-2 pr-14 border border-border rounded-md bg-background text-foreground placeholder:text-muted-foreground placeholder:text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                  value={musicPrompt}
            ...
            {/* Vibe Reference & Track Count */}
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Vibe Reference Section */}
                <div className="space-y-3">
                  <label className="text-sm font-medium text-foreground">Vibe Reference</label>
                  <div className="relative">
                    <label htmlFor="vibe-upload" className="flex items-center gap-3 p-4 rounded-lg bg-muted/50 text-sm text-foreground/70 cursor-pointer hover:bg-muted/70 transition-colors border border-border h-14">
                      <Upload className="w-4 h-4 flex-shrink-0"/>
                      <span className="flex-1 truncate">{vibeFile ? vibeFile.name : 'Upload vibe reference'}</span>
                      {vibeFile && (
                        <Button
                          type="button"
                          variant="ghost"
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-center h-14">
                    <div className="flex items-center gap-2 bg-muted/50 border border-border rounded-lg p-3 w-full justify-center">
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
                        onClick={() => setMusicTracksToGenerate(Math.max(1, musicTracksToGenerate - 1))}
                        disabled={musicTracksToGenerate <= 1}
                        className="h-8 w-8 p-0 hover:bg-muted-foreground/20 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                      ...
                      <div className="flex items-center gap-2 min-w-[80px] justify-center">
                        {isEditingTracks ? (
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!isNaN(value) && value >= 1 && value <= 10) {
                                setMusicTracksToGenerate(value);
                              }
                            }}
                            onBlur={() => setIsEditingTracks(false)}
                            onKeyDown={(e) => {
                            ...
                            autoFocus
                            className="w-12 text-center bg-muted/20 border border-border outline-none text-lg font-semibold text-foreground rounded px-2 py-1"
                            style={{ 
                          ...
                          <span 
                            className="w-8 text-center text-lg font-semibold text-foreground cursor-pointer hover:bg-muted/30 rounded px-1"
                            onClick={() => setIsEditingTracks(true)}
                          >
                            {musicTracksToGenerate}
                          </span>
                        )}
                        <span className="text-sm text-foreground/70">track{musicTracksToGenerate !== 1 ? 's' : ''}</span>
                      </div>
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        onClick={() => setMusicTracksToGenerate(Math.min(10, musicTracksToGenerate + 1))}
                        disabled={musicTracksToGenerate >= 10}
                        className="h-8 w-8 p-0 hover:bg-muted-foreground/20 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
            ...
            <Button
              className="w-full h-14 text-lg font-semibold btn-ai-gradient text-white disabled:opacity-50 disabled:cursor-not-allowed rounded-lg"

...
export function useReuseVideoclip({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && projectId) {
      ...
      console.log('Hook loading from localStorage:', {
        reuseEnabled: savedReuse,
        sharedDescription: savedShared,
        individualDescriptions: savedIndividual,
        parsedIndividual: savedIndividual ? JSON.parse(savedIndividual) : null
      });
      
      // Also check what's actually in localStorage right now
      console.log('Current localStorage content:', {
        reuseEnabled: localStorage.getItem(`musicClip_${projectId}_reuseEnabled`),
        sharedDescription: localStorage.getItem(`musicClip_${projectId}_sharedDescription`),
        individualDescriptions: localStorage.getItem(`musicClip_${projectId}_individualDescriptions`)
      });
      ...
      if (savedIndividual) {
        ...
        setIndividualDescriptions(individualDescs);
      }
      
      // Apply descriptions to tracks immediately
      if (musicTracks.length > 0) {
        if (reuseEnabled) {
          // Reuse mode: apply shared description to all tracks
          const newTrackDescriptions: Record<string, string> = {};
          musicTracks.forEach(track => {
            newTrackDescriptions[track.id] = sharedDesc;
          });
          console.log('Hook setting track descriptions (reuse mode):', newTrackDescriptions);
          // Only update if descriptions are different to prevent infinite loops
          if (JSON.stringify(trackDescriptions) !== JSON.stringify(newTrackDescriptions)) {
            setTrackDescriptions(newTrackDescriptions);
          }
        } else {
          // Individual mode: use individual descriptions
          console.log('Hook setting track descriptions (individual mode):', individualDescs);
          console.log('Current trackDescriptions:', trackDescriptions);
          console.log('Are descriptions different?', JSON.stringify(trackDescriptions) !== JSON.stringify(individualDescs));
          // Only update if descriptions are different to prevent infinite loops
          // Also don't overwrite existing data with empty objects
          const hasNonEmptyDescriptions = Object.values(individualDescs).some(desc => desc && desc.trim().length > 0);
          const shouldUpdate = JSON.stringify(trackDescriptions) !== JSON.stringify(individualDescs) && 
                              (hasNonEmptyDescriptions || Object.keys(trackDescriptions).length === 0);
          
          if (shouldUpdate) {
            console.log('Calling setTrackDescriptions with:', individualDescs);
            setTrackDescriptions(individualDescs);
          } else {
            console.log('Skipping setTrackDescriptions - descriptions are the same or empty object would overwrite existing data');
          }
          
          // Force a re-render to update validation state
          console.log('Forcing validation update for individual mode');
        }
      }
      ...
      if (typeof window !== 'undefined') {
        ...
        console.log('Hook loading from temp localStorage:', {
        ...
        if (tempReuse !== null) {
          reuseEnabled = JSON.parse(tempReuse);
          setIsReuseEnabled(reuseEnabled);
        }
        if (tempShared) {
          sharedDesc = tempShared;
          setSharedDescription(sharedDesc);
        }
        if (tempIndividual) {
          individualDescs = JSON.parse(tempIndividual);
          setIndividualDescriptions(individualDescs);
        }
        ...
        if (musicTracks.length > 0) {
          if (reuseEnabled) {
            ...
            console.log('Hook setting track descriptions (temp, reuse mode):', newTrackDescriptions);
            // Only update if descriptions are different to prevent infinite loops
            if (JSON.stringify(trackDescriptions) !== JSON.stringify(newTrackDescriptions)) {
              setTrackDescriptions(newTrackDescriptions);
            }
            ...
            // Individual mode: use individual descriptions
            console.log('Hook setting track descriptions (temp, individual mode):', individualDescs);
            // Only update if descriptions are different to prevent infinite loops
            // Also don't overwrite existing data with empty objects
            const hasNonEmptyDescriptions = Object.values(individualDescs).some(desc => desc && desc.trim().length > 0);
            const shouldUpdate = JSON.stringify(trackDescriptions) !== JSON.stringify(individualDescs) && 
  ...
  }, [projectId, musicTracks, setTrackDescriptions]);
  ...
  const getTrackValidity = useCallback((trackId: string): boolean => {
    if (isReuseEnabled) {
      ...
      // Prioritize hook's own state over parent's state for validation
      const description = individualDescriptions[trackId] || trackDescriptions[trackId] || '';
      const isValid = isValidDescription(description);
      console.log(`getTrackValidity for track ${trackId}:`, { description, isValid, individualDescriptions: individualDescriptions[trackId], trackDescriptions: trackDescriptions[trackId] });
      return isValid;
    }
  ...
  }, [isReuseEnabled, musicTracks, setTrackDescriptions, projectId]);

  // Update individual description
  const updateIndividualDescription = useCallback((trackId: string, description: string) => {
    console.log('updateIndividualDescription called:', { trackId, description, isReuseEnabled });
    if (!isReuseEnabled) {
      ...
      console.log('Setting new individual descriptions:', newIndividualDescriptions);
      ...
      if (typeof window !== 'undefined') {
        if (projectId) {
          console.log('Saving to localStorage with projectId:', projectId);
          console.log('Saving individualDescriptions:', newIndividualDescriptions);
          localStorage.setItem(`musicClip_${projectId}_individualDescriptions`, JSON.stringify(newIndividualDescriptions));
          console.log('Saved to localStorage, now reading back:', localStorage.getItem(`musicClip_${projectId}_individualDescriptions`));
        } else {
          console.log('Saving to temp localStorage');
          console.log('Saving individualDescriptions:', newIndividualDescriptions);
          localStorage.setItem('musicClip_temp_individualDescriptions', JSON.stringify(newIndividualDescriptions));
          console.log('Saved to temp localStorage, now reading back:', localStorage.getItem('musicClip_temp_individualDescriptions'));
        }
      }
    } else {
      console.log('Skipping individual description update - reuse is enabled');
    }
  ...
  const getAllDescriptions = useCallback(() => {
    ...
    console.log('getAllDescriptions called, returning:', result);
    return result;
